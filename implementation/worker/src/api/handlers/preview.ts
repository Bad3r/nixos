...
export class PreviewHandler {
   ...
   */
  async handleCreatePreview(c: Context<{ Bindings: Env }>) {
    try {
      // Get validated data
      const validated = c.get('validated') as any;
      const { prNumber, modules, branch, sha } = validated.body;

      // Generate preview key
      const previewKey = `${this.PREVIEW_PREFIX}${prNumber}`;
      const previewMetaKey = `${this.PREVIEW_PREFIX}${prNumber}:meta`;
      ...
      // Check size
      const dataSize = new TextEncoder().encode(JSON.stringify(previewData)).length;
      if (dataSize > this.MAX_PREVIEW_SIZE) {
        return c.json({
          error: 'Preview data too large',
          maxSize: this.MAX_PREVIEW_SIZE,
          actualSize: dataSize,
        }, 413);
      }
      ...
      const metadata = {
        ...
        expiresAt: new Date(Date.now() + this.PREVIEW_TTL * 1000).toISOString(),
        previewUrl,
        apiPreviewUrl,
        size: dataSize,
      };
      ...
      await c.env.CACHE.put(
        ...
        {
          expirationTtl: this.PREVIEW_TTL,
          metadata: {
            type: 'preview-meta',
   ...
   */
  async handleGetPreview(c: Context<{ Bindings: Env }>) {
    try {
   ...
   * Delete a preview
   */
  async handleDeletePreview(c: Context<{ Bindings: Env }>) {
    try {
      const prNumber = c.req.param('pr');
      ...
      const previewKey = `${this.PREVIEW_PREFIX}${prNumber}`;
      const previewMetaKey = `${this.PREVIEW_PREFIX}${prNumber}:meta`;
      const previewChunksPrefix = `${this.PREVIEW_PREFIX}${prNumber}:chunk:`;
   ...
   */
  async handleListPreviews(c: Context<{ Bindings: Env }>) {
    try {
      ...
      for (const prNumber of index.activePreviews || []) {
        const metaKey = `${this.PREVIEW_PREFIX}${prNumber}:meta`;
   ...
   */
  async handleUpdatePreview(c: Context<{ Bindings: Env }>) {
    try {
      const prNumber = c.req.param('pr');
      const updates = await c.req.json<{ modules?: any[]; branch?: string; sha?: string }>();
   ...
   */
  private async storePreviewData(env: Env, key: string, data: any): Promise<void> {
