...
let
  ...
  testResults = {
    ...
    testEnumExtraction = let
      result = extractModules.extractType (lib.types.enum [ "foo" "bar" "baz" ]);
    in {
      assertion =
        result.type == "option-type" &&
        result.name == "enum" &&
        result.values == [ "foo" "bar" "baz" ];
      message = "Should extract enum values";
    };

    # Test 11: Function type handling
    testFunctionType = let
      result = extractModules.extractType (lib.types.functionTo lib.types.str);
    in {
      assertion =
        result.type == "option-type" &&
        result.name == "functionTo" &&
        result.returnType.name == "str";
    ...
    testComplexNestedSubmodule = let
    ...
    in {
      assertion =
        result.declarations != [] &&
        lib.any (d: lib.hasInfix "test.nix" d.file) result.declarations;
      message = "Should extract option declarations";
    };

    # Test 18: Type with custom check
    testTypeWithCustomCheck = let
  ...
  # Run all tests
  runTests = lib.mapAttrs (name: test:
