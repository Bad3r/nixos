...
def latest_release() -> tuple[str, set[str]]:
    ...
    msg = f"Could not determine latest brave-origin version with {REQUIRED_PLATFORM} release asset."
    raise RuntimeError(msg)
...
def available_archive_assets(version: str, release_assets: set[str]) -> dict[str, str]:
    ...
    if REQUIRED_PLATFORM not in archives:
        msg = f"Missing expected asset: {release_asset_name(REQUIRED_PLATFORM, version)}"
        raise RuntimeError(msg)
    return archives
...
def render_default_nix(
    ...
    lines = [
        ...
        (
            "# TODO(https://github.com/NixOS/nixpkgs/pull/511131): replace "
            "this directory with `pkgs.brave-origin` in the"
        ),
