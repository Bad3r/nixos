_: {
  perSystem = _: {
    files.file.".githooks/post-checkout".text = ''
      #!/usr/bin/env bash
      # Managed by Nix. Edit modules/development/git-hooks-post-checkout.nix
      # then run `nix develop path:. -c write-files` to refresh. This hook fires
      # on `git worktree add`, and a clean linked worktree is not fetchable as a
      # git+file flake, so the bare form fails exactly where this text is read.
      #
