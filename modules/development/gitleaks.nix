    ...
    in
    {
      files.file = {
        ".gitleaks.toml".text = ''
          ...
          # Superproject pass only. Gitlink passes read
          # .gitleaks-secrets.toml for the pinned private gitlinks and
          # .gitleaks-gitlink.toml for every other gitlink, because path
          # patterns are matched against a File rooted at whichever tree is
          # being scanned.
          [extend]
          useDefault = true
        ...
        ".gitleaks-secrets.toml".text = ''
          # Read only by the pinned private gitlinks in
          # modules/meta/hooks/gitleaks.nix.
