/*
  Claude Code environment variables. Every entry in `vars` reaches
...
in
{
  # Exported by the launchers unless launchOnlyEscape is set, never written to
  # settings.json: either name blocks the feature-flag evaluation `claude rc`
  # needs, and only a value a wrapper set can be lifted by claude-rc.
  launchOnly = [
    "CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC"
    "DISABLE_TELEMETRY"
  ];
  # Read only by this repo's launchers; Claude Code never looks at it.
  ...
  vars = {
    ...
    # Repoints the haiku alias, which also backs titles, summarization, and classifiers.
    ...
    # Effort, thinking, context, memory
    ...
    # Carries max, which the persisted effortLevel setting rejects.
    ...
    # Inert until CLAUDE_CODE_ENABLE_TELEMETRY is set; one of the two OTel metric attributes included by default.
    ...
    # Set by the launcher to the controlled bash; a value here would bypass the rm shim.
    # CLAUDE_CODE_SHELL = "";
    ...
    # Terminal, rendering, accessibility
    ...
    # Read-only (exported by Claude Code; do not set)
    ...
    # === Undocumented: read by the 2.1.222 binary, absent from the docs ===
