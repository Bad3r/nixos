_: {
  flake.lib.nixos.hosts.songbird = {
    ...
    # two onboard NICs. firewall.nix warns on an unpinned kernel name and the
    # warning clears once a pin backs the entry, but it cannot tell a right
    # kernel name from a wrong one, so the pin is still the guarantee.
