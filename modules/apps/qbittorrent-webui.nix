    ...
    let
      ...
      handler = pkgs.writeShellApplication {
        ...
        text = ''
          ...
          # torrents/add reports failure in the body, not the status: it
          # answers 200 "Fails." for a duplicate, a rejected magnet, or a bad
          # save path, and 415 only for an unparsable torrent file.
          added() {
            [ "$1" = "Ok." ]
          }
          ...
          for item in "$@"; do
            case "$item" in
              magnet:*)
                response=$(curl -fsS --data-urlencode "urls=$item" "$api") || response=""
                if added "$response"; then
              ...
              *)
                ...
                quoted=''${path//\\/\\\\}
                quoted=''${quoted//\"/\\\"}
                response=$(curl -fsS -F "torrents=@\"$quoted\"" "$api") || response=""
                if added "$response"; then
