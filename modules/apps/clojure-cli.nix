...
let
  cfg = config.programs.clojure.extended;
  ClojureModule = {
    options.programs.clojure.extended = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = lib.mdDoc "Whether to enable clojure.";
      };
...
in
{
  flake.nixosModules.apps.clojure = ClojureModule;
}
