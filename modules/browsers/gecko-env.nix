/*
  ...
  (modules/hardware/nvidia-gpu.nix, see that option's Xid 31 rationale) never
  installs it, so the NVIDIA VA-API variables would override the host's iHD
  routing and weaken the RDD process sandbox for a decode path that does not
  exist.
