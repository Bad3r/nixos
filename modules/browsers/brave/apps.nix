/*
  ...
  Options:
    ...
    /etc/brave/policies/managed/dns-over-https.json: Resolver policy file shared with the brave-origin module.
...
let
  BraveModule =
    ...
    let
      ...
      # resolver file is shared with Brave Origin through one policy directory.
      inherit (import ../_chromium-policies.nix)
        managedDefaultSearchProvider
        managedDnsOverHttps
        braveDnsOverHttpsEtc
        ;
      ...
      restatedDnsOverHttpsKeys = lib.filter (key: cfg.managedPolicies ? ${key}) (
        lib.attrNames managedDnsOverHttps
      );
    in
    {
      options.programs.brave.extended = {
        ...
        managedPolicies = lib.mkOption {
          ...
          description = ''
            ...
            DnsOverHttpsMode and DnsOverHttpsTemplates are rejected here: they live in the
            dns-over-https.json this module shares with brave-origin.
          '';
        };
      };
      ...
      config = lib.mkIf cfg.enable {
        assertions = [
          {
            assertion = restatedDnsOverHttpsKeys == [ ];
            message = ''
              programs.brave.extended.managedPolicies restates ${lib.concatStringsSep ", " restatedDnsOverHttpsKeys}:
              /etc/brave/policies/managed/dns-over-https.json owns the resolver policy for every Brave channel,
              and Chromium resolves a key set in two files of one directory by file order, not by this option.
            '';
          }
        ];
