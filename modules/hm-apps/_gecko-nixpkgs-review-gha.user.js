...
const user =
	document
		.querySelector("header.GlobalNav button[data-login]")
		?.getAttribute("data-login") ?? null;
...
const prTrackers = [
	{
		name: "nixpk.gs",
		toUrl: (pr) => `https://nixpk.gs/pr-tracker.html?pr=${pr}`,
	},
	{
		name: "ocfox.me",
		toUrl: (pr) => `https://nixpkgs-tracker.ocfox.me/?pr=${pr}`,
	},
];
...
const getPrDetails = (pr) => {
	...
	const commits = [
		...
		const match = /\/NixOS\/nixpkgs\/pull\/(\d+)\/commits\/([0-9a-f]+)$/i.exec(
			href,
		);
	...
	const labels = [...document.querySelectorAll("div.js-issue-labels > a")].map(
		(x) => x.innerText,
	);
	const author = document
		.querySelector(".js-discussion > :first-child a.author")
		.href.split("/")
		.at(-1);
	...
	const state = document
		.querySelector(
			"div[data-component=TitleArea] div[data-component=PH_LeadingVisual] span, span.State",
		)
		.innerText.trim()
		.toUpperCase();
...
const setupActionsPage = async () => {
	const match =
		/^https:\/\/github.com\/([^/]+\/[^/]+)\/actions\/workflows\/review.yml#dispatch:(.*)$/.exec(
			location.href,
		);
	...
	const setBranch = async (branch) => {
		(await query(document, "details .workflow-dispatch")).classList.add(
			"old-branch",
		);
		document
			.querySelector(
				"details .workflow-dispatch .branch-selection > details > summary",
			)
			.click();
		(
			await query(
				document,
				`ref-selector[type=branch] button[value="${branch}"]`,
			)
		).click();
		while (
			(await query(document, "details .workflow-dispatch")).classList.contains(
				"old-branch",
			)
		)
			await sleep(100);
	...
	const setInput = (name, value) => {
		...
		if (input.type === "checkbox") {
			if (!["true", "false"].includes(value)) {
				alert(
					`workflow_dispatch input '${name}' expects a boolean ('true' or 'false) but is set to '${value}'`,
				);
				return;
			}
	...
	document
		.querySelector("details .workflow-dispatch button[type=submit]")
		.focus();
};

const setupPrPage = async () => {
	const match =
		/^https:\/\/github.com\/NixOS\/nixpkgs\/pull\/(\d+)([?#].*)?$/i.exec(
			location.href,
		);
	...
	if (actions.querySelector(".run-nixpkgs-review") === null && repo) {
		...
		btn.onclick = () => {
			const params = new URLSearchParams({
				...reviewDefaults(getPrDetails(pr)),
				pr,
			});
			window.open(
				`https://github.com/${repo}/actions/workflows/review.yml#dispatch:${params}`,
			);
		};
	}
	...
	if ((!hasLinuxRebuilds && !hasDarwinRebuilds) || state == "MERGED") {
		actions
			.querySelector(".run-nixpkgs-review")
	...
	if (actions.querySelector(".goto-pr-tracker") === null) {
		for (const { name, toUrl } of prTrackers) {
			...
			btn.innerText =
				prTrackers.length === 1 ? "PR Tracker" : `PR Tracker (${name})`;
			actions.prepend(btn);
			btn.onclick = () => {
...
new MutationObserver(setupPrPage).observe(document, {
	subtree: true,
	childList: true,
});
