...
def main():
    ...
    def resolve_resources_hash_with_nix() -> str | None:
        system = run(
            [
                "nix",
                "eval",
        ...
        attr = f".#packages.{system}.logseq-fhs-unwrapped"
        ...
        match = re.search(r"got:\s+sha256-([A-Za-z0-9+/=]+)", build_proc.stderr)
        ensure(
            match,
            f"unable to extract resources hash from nix build output\nstdout: {build_proc.stdout}\nstderr: {build_proc.stderr}",
        )

        sri = f"sha256-{match.group(1)}"
        base32 = run(
            [
                "nix",
                "hash",
    ...
    legacy_resources_workspace = json.loads(resources_workspace_json.read_text())

    static_dir = logseq_repo / "static"
    static_package_json = static_dir / "package.json"
    static_yarn_lock = static_dir / "yarn.lock"
    ensure(static_package_json.exists(), f"missing static package.json at '{static_package_json}'")
    ensure(static_yarn_lock.exists(), f"missing static yarn.lock at '{static_yarn_lock}'")

    resources_workspace_data = {
        "packageJson": static_package_json.read_text(),
        "yarnLock": static_yarn_lock.read_text(),
    }
    ...
    with tempfile.TemporaryDirectory() as patched_dir_str:
        ...
        resolved_hash = resolve_resources_hash_with_nix()
        if resolved_hash:
            workspace_payload["hash"] = resolved_hash
            resources_workspace_json.write_text(
                json.dumps(workspace_payload, indent=2) + "\n"
            )
