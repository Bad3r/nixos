...
import. `environment.variables` reaches shells and whatever sources
`/etc/profile`, and the i3 module imports only session runtime names such
as DISPLAY and XAUTHORITY into the user manager. The default-app hooks in
`modules/xdg/mime.nix` route through `sessionEnv`, which sets both the
NixOS session variable and the Home Manager one.
