#!/usr/bin/env bash

set -euo pipefail

if [[ ${1-} ]]; then
  REPO_ROOT=$(realpath "$1")
else
  REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")"/../.. && pwd)
fi

REGISTRY="$REPO_ROOT/docs/RFC-0001/manifest-registry.json"

if [[ ! -f $REGISTRY ]]; then
  echo "host-package-guard: manifest registry not found at $REGISTRY" >&2
  exit 1
fi

export NIX_CONFIG="${NIX_CONFIG:-experimental-features = nix-command flakes}"

tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

jq -c '.[]' "$REGISTRY" >"$tmpdir/entries.json"

status=0

normalize_manifest() {
  python -c 'import json, re, sys
from pathlib import Path
path = Path(sys.argv[1])
data = json.loads(path.read_text())
def normalize(name: str) -> str:
    s = str(name)
    s = re.sub(r"^([0-9a-z]{32})-", "", s)
    return re.sub(r"-[0-9].*$", "", s)
for item in sorted({normalize(entry) for entry in data}):
    print(item)
' "$1"
}

normalize_actual() {
  python -c 'import json, re, sys
data = json.loads(sys.stdin.read())
def normalize(name: str) -> str:
    s = str(name)
    s = re.sub(r"^([0-9a-z]{32})-", "", s)
    return re.sub(r"-[0-9].*$", "", s)
for item in sorted({normalize(entry) for entry in data}):
    print(item)
'
}

while IFS= read -r entry; do
  host=$(jq -r '.host // empty' <<<"$entry")
  manifest_rel=$(jq -r '.manifest' <<<"$entry")

  if [[ -z $manifest_rel ]]; then
    echo "host-package-guard: registry entry missing manifest" >&2
    status=1
    continue
  fi

  if [[ -z $host ]]; then
    echo "host-package-guard: skipping entry without host binding ($manifest_rel)" >&2
    continue
  fi

  manifest="$REPO_ROOT/$manifest_rel"
  if [[ ! -f $manifest ]]; then
    echo "host-package-guard: manifest $manifest not found" >&2
    status=1
    continue
  fi

  manifest_sorted="$tmpdir/${host}-expected.txt"
  normalize_manifest "$manifest" >"$manifest_sorted"

  expr=$(cat <<EOF
let
  flake = builtins.getFlake "${REPO_ROOT}";
  pkgs = flake.nixosConfigurations."${host}".config.environment.systemPackages;
  normalize = pkg:
    let
      name =
        if pkg ? pname then pkg.pname else
          let
            raw = builtins.parseDrvName (
              builtins.baseNameOf (builtins.unsafeDiscardStringContext (toString pkg))
            );
            m = builtins.match "^[0-9a-z]+-(.*)$" raw.name;
          in if m == null then raw.name else builtins.head m;
    in name;
in builtins.map normalize pkgs
EOF
)
  actual_sorted="$tmpdir/${host}-actual.txt"
  nix eval --json --impure --no-warn-dirty --accept-flake-config --expr "$expr" \
    | normalize_actual >"$actual_sorted"

  missing=$(comm -23 "$manifest_sorted" "$actual_sorted" || true)
  unexpected=$(comm -13 "$manifest_sorted" "$actual_sorted" || true)

  if [[ -n $missing || -n $unexpected ]]; then
    status=1
    echo "host-package-guard: ${host} diverges from $manifest_rel" >&2
    if [[ -n $missing ]]; then
      echo "  missing:" >&2
      echo "$missing" >&2
    fi
    if [[ -n $unexpected ]]; then
      echo "  unexpected:" >&2
      echo "$unexpected" >&2
    fi
  fi
done <"$tmpdir/entries.json"

exit $status
