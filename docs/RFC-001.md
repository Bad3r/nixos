...
3. **Refactor role modules.** Update `modules/roles/{dev,media,net}.nix` so they:
   ...
   - Retain or add alias modules (`flake.nixosModules."role-*"`).
...
4. **Install guardrails.**
   ...
   - Add a CI assertion under `modules/meta/ci.nix` (or equivalent) that validates helper presence and role alias lists during automated checks.
