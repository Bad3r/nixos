...
| `sops -e -i secrets/file.yaml.example`           | The deny-by-default rule also matches `*.example`, so SOPS encrypts a template that the cleartext gate intentionally exempts. Edit `*.example` files directly and never run SOPS on them.       |
