...
- `.sops.yaml` is generated by `modules/security/sops-policy.nix` through the `files` module. The canonical policy currently contains:
  ...
  - Three specific rules ahead of the deny-by-default catch-all: `secrets/act.yaml` (only the `github_token` field encrypted), `secrets/r2.env`, and `secrets/r2.yaml`. The final `secrets/.*` rule covers every remaining path under `secrets/`, including extensionless names and dotfiles. SOPS applies the first matching rule, so the catch-all must stay last.
  - The specialized `secrets/act.yaml` rule is end-anchored, so names such as `act.yaml.bak` reach the deny-by-default rule instead of inheriting its narrow `github_token`-only encryption scope.
  - `checks.<system>.secrets-no-cleartext` is the evaluation-time gate for initialized `secrets/` trees. It validates the security-critical SOPS policy contract, including the exact host key anchor, top-level creation-rule item sequence, and expected indentation shapes so an indentless YAML sequence cannot hide a rule from the fixtures, confirms the submodule `.gitignore` retains the exact active `**/decrypted_*` and `*.dec.*` lines used by the cleartext exemptions, rejects active negation lines that could re-include an exempt path, prunes `.git` and `decrypted_*` directories before recursion, walks the remaining regular files, resolves filesystem entries that report an unknown directory type, rejects unsupported special files on non-exempt paths and non-exempt symlinks with their relative paths and remediation, requires a SOPS MAC footer field (`mac:`, JSON `"mac":`, dotenv `sops_mac=`, or an INI `[sops]` `mac =` field) plus `lastmodified` metadata and the canonical host recipient, and fails closed for non-exempt cleartext. The marker scan uses bounded chunks and bounded INI line probes so large cleartext records still reach the named-path error. The local conventions exempt `*.example` templates, `decrypted_*` path segments and their descendants, `*.dec.*` artifacts at any depth, exact Git metadata basenames (`.gitignore`, `.gitattributes`, `.gitmodules`, and `.gitkeep`), and paths under a `.git` segment. Names such as `.git-credentials` and bare `.dec` suffixes remain encrypted. Encrypted `*.example` payloads are rejected as accidental template encryption. Secretless CI checks all generated files through `managed-files-synced` and checks the policy, exemption, marker, and directory-walk fixture tables, but skips payload scanning because the private submodule is unavailable.
  - The initialized cleartext gate scans every reachable nested `.gitignore` for active `!` rules as well as checking the root exemption patterns. Nested negations are rejected because Git applies the deeper ignore file and can re-include a `decrypted_*` or `*.dec.*` path.
  - The ignore-rule parser applies Git's trailing-whitespace semantics, so cosmetic unescaped spaces do not invalidate the required exemption patterns while escaped trailing spaces remain non-canonical.
  - Directory traversal prunes `.git`, `decrypted_*`, and `*.dec.*` directories before recursion, keeping the cleartext gate aligned with the submodule's ignored descendants.
...
1. Generate the replacement identity:
   ...
   ```
2. Replace `host_pub_key` in `modules/security/sops-policy.nix` and the
...
| `sops -e secrets/file.tmp > secrets/file.yaml`   | The deny-by-default rule matches `.tmp`, but the plaintext source remains under `secrets/` and the cleartext gate rejects it. Edit outside `secrets/`, then encrypt in place with `sops -e -i`.               |
| `sops -e -i secrets/file.yaml.example`           | The deny-by-default rule also matches `*.example`, so SOPS encrypts a template that the cleartext gate rejects as accidental template encryption. Edit `*.example` files directly and never run SOPS on them. |
