...
- `.sops.yaml` is generated by `modules/security/sops-policy.nix` through the `files` module. The canonical policy currently contains:
  ...
  - `checks.<system>.secrets-no-cleartext` is the evaluation-time gate for initialized `secrets/` trees. It validates the security-critical SOPS policy contract, walks regular files and symlinks, requires a SOPS MAC footer field (`mac:`, JSON `"mac":`, or dotenv `sops_mac=`) plus `lastmodified` metadata, and fails closed for non-exempt cleartext. Dangling or directory symlinks fail with their relative path and remediation. The local conventions exempt `*.example` templates, `decrypted_*`, `*.dec.*` artifacts at any depth, exact Git metadata basenames (`.gitignore`, `.gitattributes`, `.gitmodules`, and `.gitkeep`), and paths under a `.git` segment. Names such as `.git-credentials` and bare `.dec` suffixes remain encrypted. Encrypted `*.example` payloads are rejected as accidental template encryption. Secretless CI checks all generated files through `managed-files-synced` and checks the policy, exemption, and marker fixture tables, but skips payload scanning because the private submodule is unavailable.
