...
- `.sops.yaml` is generated by `modules/security/sops-policy.nix` through the `files` module. The canonical policy currently contains:
  ...
  - `checks.<system>.secrets-no-cleartext` is the evaluation-time gate for initialized `secrets/` trees. It walks regular files and symlinks, requires both the full `ENC[AES256_GCM,` marker and SOPS `lastmodified` metadata, and fails closed for non-exempt cleartext. The local conventions exempt `*.example` templates, `decrypted_*`, `*.dec.*` artifacts at any depth, exact Git metadata basenames (`.gitignore`, `.gitattributes`, `.gitmodules`, and `.gitkeep`), and paths under a `.git` segment. Names such as `.git-credentials` and bare `.dec` suffixes remain encrypted. Encrypted `*.example` payloads are rejected as accidental template encryption. Secretless CI compares the committed `.sops.yaml` with the `files`-module source and checks the exemption and marker fixture tables, but skips payload scanning because the private submodule is unavailable.
