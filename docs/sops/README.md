...
- `checks.<system>.secrets-no-cleartext` is the evaluation-time gate for initialized `secrets/` trees. It walks regular files and symlinks, requires both the full `ENC[AES256_GCM,` marker and SOPS `lastmodified` metadata, and fails closed for non-exempt cleartext. The local conventions exempt `*.example` templates, `decrypted_*` and `*.dec*` working artifacts, and exact Git metadata names (`.git`, `.gitignore`, `.gitattributes`, `.gitmodules`, and `.gitkeep`). Names such as `.git-credentials` remain encrypted. Secretless CI checks policy parity but skips payload scanning because the private submodule is unavailable.
