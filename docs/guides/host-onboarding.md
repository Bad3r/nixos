...
3. Commit the host's files and land them on the branch the target checks out, then boot the generation from that checkout on the target machine without switching its running system:
   ...
   `path:` copies `secrets/` as it finds it instead of excluding it; `git worktree add` does not initialize submodules on its own, but this repository's `post-checkout` hook (`modules/development/git-hooks-post-checkout.nix`) runs `git submodule update --init --recursive` on the checkout `git worktree add` performs, so a linked worktree sharing this repository's `core.hooksPath` already stages `sops.secrets."act/github_token"`; `modules/security/secrets.nix` declares that secret on `builtins.pathExists` alone, with no `sopsRuntimeReady` gate, so the new host's first boot fails sops-nix activation before [Host secrets and handoff](host-onboarding-secrets.md) installs the age identity.
