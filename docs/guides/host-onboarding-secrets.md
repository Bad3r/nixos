...
2. Switch every host that carries the fleet SSH config. This is every host with `programs.tailscale.extended` on, including the incoming and outgoing primary. `modules/networking/ssh-hosts.nix` renders `~/.ssh/hosts/tailscale` at build time, so an unswitched host retains the outgoing primary's address.
