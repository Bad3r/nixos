...
The `qbittorrent-webui` handler opens each magnet link or `.torrent` file in the add dialog of Songbird's Web UI.
The dialog is where the category and save path are chosen before the torrent is added.
