...
- **Single source of truth for agent config.** `modules/agents/system-prompt.nix`
  renders both `CLAUDE.md` and `AGENTS.md`; `modules/agents/mcp.nix` compiles
  per-client MCP server lists. Edit the source, not the rendered output.
...
| 12  | Theming with Stylix                   | `modules/stylix/stylix.nix`, `modules/stylix/github-dark-dimmed.base16.yaml`                                                                |
...
- `modules/agents/system-prompt.nix`: `flake.lib.agents.systemPrompt` with ordered
  sections + render function (renders `CLAUDE.md` / `AGENTS.md`).
...
- root `README.md`, `CLAUDE.md`, `AGENTS.md`: project + agent guides (the latter
  two are generated).
...
- `modules/networking/ssh.nix`, `modules/hosts/common/usbguard.nix`:
  security-sensitive (usbguard is currently force-disabled pending an LSM-stacking
  fix).
