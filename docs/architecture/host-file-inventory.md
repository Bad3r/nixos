...
| `modules/songbird/qbittorrent.nix`       | qBittorrent service in the `torrent` network namespace behind a Proton VPN WireGuard tunnel, with NAT-PMP port renewal and a Web UI proxy for LAN and Mesh clients                                                                 |
| `modules/songbird/qbittorrent-webui.nix` | Desktop handler enable and the patched copy of the service's own Web UI, served in place of its built-in one                                                                                                                       |
| `modules/songbird/apps-enable.nix`       | Per-host overrides over the common app baseline                                                                                                                                                                                    |
| `modules/songbird/cloudflare-warp.nix`   | WARP `serviceMode = "warp"`: systemd-resolved sends every lookup to the WARP resolver while its tunnel is up                                                                                                                       |
| `modules/songbird/policy.nix`            | Registry data under `flake.lib.nixos.hosts.songbird` (readiness gates and per-host values)                                                                                                                                         |
| `modules/songbird/services.nix`          | Host-divergent services                                                                                                                                                                                                            |
...
| `modules/tpnix/cloudflare-warp.nix`      | WARP `serviceMode = "tunnelonly"`: traffic only, NetworkManager's dnsmasq keeps the private-host mappings                                                        |
...
| `modules/tpnix/policy.nix`               | Registry data under `flake.lib.nixos.hosts.tpnix` (readiness gates, per-host values, and private DNS host secret keys)                                           |
