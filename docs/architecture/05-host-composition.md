...
Registry entries also carry fleet endpoint data. A host records its Cloudflare Mesh device address as `meshIp` in `modules/<host>/policy.nix` after enrolling through `modules/apps/cloudflare-warp.nix`; `modules/networking/ssh-hosts.nix` derives one `<host>.local` alias per registered host (excluding self) and, on hosts that run WARP, one `<host>.warp` alias per registered host with a `meshIp`; `modules/hosts/common/registry.nix` rejects a `meshIp` outside `100.96.0.0/12`, `modules/networking/ssh-hosts.nix` rejects one that two hosts share, and `modules/hosts/common/ssh-known-hosts.nix` pins the host key for that address as well. `modules/apps/tailscale.nix` keeps the `primary` and `tailnetIp` mechanism for a host that enables Tailscale: at most one registry host may be primary, it must carry a non-empty `tailnetIp` string, and no primary leaves `sshHostName` unset.
