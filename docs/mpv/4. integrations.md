...
`mpvScripts.mpris` is loaded through HM's `programs.mpv.scripts`, not through
the NixOS `extraScripts` list, because mpv only auto-loads scripts that the HM
layer wires in. See [scripts-and-bindings.md](<3. scripts-and-bindings.md>) for the
distinction.
