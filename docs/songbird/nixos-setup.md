...
| `modules/songbird/services.nix`                                 | Samba media share (from `secrets/songbird.yaml`) admitting loopback and the approved LAN CIDRs only (`hosts allow`/`hosts deny`), on-demand `samba.target` with WS-Discovery bound to it, coredump retention, power-profiles-daemon forced to performance (replacing system76-power), cloudflared, WARP headless, LACT, system76-scheduler, printing off                                                                                                                 |
