...
4. Initialize the secrets submodule (`git submodule update --init --recursive`;
   ...
   `node_modules` and `.venv` trees), and every run is killed by the 20-minute
   ...
   orphaned lock and begins again. The listing itself is the cost: under
   rclone's default `size,modtime` comparison every S3 object needs one HEAD
   request for its modtime, and the `r2-flake` module exposes no way to
   change the comparison or exclude paths. Upstream fix requested in
   <https://github.com/Bad3r/nix-R2-CloudFlare-Flake/issues/150>; the
   consumer-side changes wait in
   <https://github.com/Bad3r/nixos/issues/477>. See the Open Items table.
...
| `r2-bisync-docs` first sync | Blocked upstream: every run since the first boot hits the 20-minute `TimeoutStartSec` during the initial listing and restarts from scratch; the `r2-flake` module exposes no bisync compare, exclude, or extra-args option (<https://github.com/Bad3r/nix-R2-CloudFlare-Flake/issues/150>), and the consumer-side changes are tracked in <https://github.com/Bad3r/nixos/issues/477> | `modules/lib/r2-runtime.nix`, `/var/lib/r2-sync-docs/bisync/`             |
