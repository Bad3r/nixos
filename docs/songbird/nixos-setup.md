...
Decision-complete plan for bringing the `songbird` host into this repository:
dual-boot layout, firmware settings, install sequence, the per-host module
files, and validation. Hardware facts live in
[project-songbird.md](project-songbird.md); the generic onboarding mechanics
live in the [Host Onboarding Runbook](../guides/host-onboarding.md). Decisions
below are locked.

Status: Phases N0, N1, and N2 are done. Disk A boots this repository's
`songbird` configuration (first boot 2026-09-01: CachyOS kernel, `cryptroot`,
`cryptswap` and `data` open, `/data` and `/shared` mounted),
`modules/songbird/` holds the hardware truth harvested from the machine, the
age identity plus the initialized `secrets/` submodule have the sops and R2
runtimes live, the host SSH key is pinned, and songbird is the primary fleet
endpoint on the tailnet. Since 2026-09-05 no `/shared` mount is declared: disk
W becomes the Windows disk (decision 4), and the shared drive moves to an SSD
that is not chosen yet (decision 5). Where the machine differs from the plan,
the tables below carry the as-built values and note the plan value in place.
Windows phases N3 and N4 follow.
...
| 13  | GPU wiring via `flake.nixosModules.nvidia-gpu`: `open = true` (NVIDIA's open kernel modules, mandatory on Blackwell), production driver branch (>= 570 required), `nouveau` blacklisted, `vaapi.backend = "nvidia"` with `"intel-media"` as the documented fallback. This VA-API wiring alone never reached Gecko: Firefox/LibreWolf blocklist hardware video decoding on every NVIDIA device (`FEATURE_HARDWARE_VIDEO_DECODING_NO_LINUX_NVIDIA`) regardless of the working libva backend, so `modules/browsers/_gecko-prefs.nix` additionally force-enables `media.hardware-video-decoding.force-enabled` wherever `services.xserver.videoDrivers` carries `"nvidia"`. |
| 14  | songbird is the primary fleet endpoint (`primary = true` and `tailnetIp = "100.120.100.117"` in its `policy.nix`); the tailscale SSH default and the ssh-hosts aliases on the other hosts follow the registry.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 16  | Steam stays enabled on the NixOS side too (`programs.steam.extended.enable`); Proton covers casual Linux-side gaming.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
volume goes away when Phase N3 installs Windows on this disk.
...
done; N2 is the first switch from this repository and is done; N3 and N4
cover the Windows install.
...
### Phase N2: Fleet integration (first switch from this repository) (done)
...
`worktree-prune` timer and `programs.nh.flake` expect. Every step is done
(first boot into this configuration on 2026-09-01); the text stays as the
...
3. Provision the canonical age identity (sops), per
   [SOPS usage](../sops/README.md) Host Preparation: copy from
...
4. Initialize the secrets submodule (`git submodule update --init --recursive`;
   ...
   on-demand Samba media share; without that key `services.nix` warns and
   skips the share.
   ...
   Observed after the first boot: `r2-bisync-docs.service` had not completed
   a run. Its bisync state under `/var/lib/r2-sync-docs/bisync/` is fresh on
   this host, `/data/Docs` holds 4.9 GB in 32,558 files (30,528 of them under
   `node_modules` and `.venv` trees), and every run was killed by the shared
   20-minute `TimeoutStartSec` before the initial listing finished; the timer
   then started the next run, which cleared the orphaned lock and began
   again. The listing itself is the cost: under rclone's default
   `size,modtime` comparison every S3 object needs one HEAD request for its
   modtime, and the `r2-flake` module exposes no way to change the comparison
   or exclude paths. As a temporary mitigation, the docs profile in
   `modules/lib/r2-runtime.nix` carries a 6-hour `bisyncStartTimeout` (the
   other profiles keep 20 minutes) so a first sync can finish; every later
   run still pays the full listing. Upstream fix requested in
   <https://github.com/Bad3r/nix-R2-CloudFlare-Flake/issues/150>; the
   consumer-side changes wait in
   <https://github.com/Bad3r/nixos/issues/477>. Return the docs bound to 20
   minutes once both land. See the Open Items table.

5. Tailnet join is done: songbird's assigned address is `100.120.100.117`,
   set as `tailnetIp` in `policy.nix`, with `primary = true` alongside it
...
7. The validation ladder ran and the PR merged.

### Phase N3: Final Windows installation
...
### Phase N4: Shared BitLocker conversion (optional)
...
`modules/songbird/` carries the following per-host files:
...
| `modules/songbird/services.nix`                                 | Samba media share (from `secrets/songbird.yaml`) admitting loopback and the approved LAN CIDRs only (`hosts allow`/`hosts deny`), on-demand `samba.target` with WS-Discovery bound to it, coredump retention, power-profiles-daemon forced to performance, cloudflared, WARP headless, LACT, printing off                                                                                                                                                                         |
...
| `modules/songbird/gnome-keyring.nix`, `pass-secret-service.nix` | gnome-keyring off, `pass` as the secret service                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `modules/songbird/mpv.nix`                                      | `gpu-api = "opengl"` override; drop once Vulkan is verified on the 5080                                                                                                                                                                                                                                                                                                                                                                                                           |
| `modules/songbird/ssh.nix`                                      | `services.openssh.enable` override and pinned host ed25519 public key                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Tailnet IPv4                | Done: `100.120.100.117`; songbird is the primary fleet endpoint                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `policy.nix` `tailnetIp`, `primary`                                                 |
...
| Windows disk                | Decided 2026-09-05: W (SN720); the `/shared` mount it carried left `hardware-config.nix` ahead of the install                                                                                                                                                                                                                                                                                                                                                                                                            | Phase N3                                                                            |
...
| Shared partition PARTUUID   | Only if Phase N4 converts the shared drive to BitLocker                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `hardware-config.nix` crypttab entry                                                |
| BitLocker keys              | Windows BitLocker setup (Phases N3, N4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Password manager; shared-drive password also to `/var/lib/secrets/shared-bitlk.key` |
...
1. Windows never hibernates and never fast-starts (`powercfg /h off`, Phase
   N3). This keeps W and the shared drive clean for every boot.
