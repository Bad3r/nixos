...
- Cloudflare WARP enrolls with the host's service token in `warp` mode, so the WARP resolver serves DNS through the `/etc/resolv.conf` that `warp-svc` writes, since nothing here serves private-host mappings: [cloudflare-warp.nix](../../modules/songbird/cloudflare-warp.nix).
