...
- In `warp` mode systemd-resolved sends every lookup to the client's local DNS proxy while the `CloudflareWARP` tunnel exists, and to the link servers once it is gone.
