...
Cause: the split-tunnel exclude list still covers the Mesh allocation, Cloudflare Mesh is off for
client devices, or the app is off on the host.
Diagnostic: `warp-cli --accept-tos settings`, `sudo iptables -S nixos-fw | grep CloudflareWARP`,
and `nc -vz <mesh-address> 22` from a peer.
...
## A `<host>.internal` alias or Mesh address is unreachable

Cause: the peer has no encrypted `mesh.hosts.<host>` value, its
`cloudflareWarpMeshAddressReady` flag is false, this host has not switched since the value changed,
or the direct Mesh route is down.
Diagnostic: `getent ahostsv4 <host>.internal` must print the peer's recorded address, and
`ping -c 1 <address>` separates name resolution from Mesh transport.
Fix: store the address from `ip -4 addr show CloudflareWARP` in
`secrets/cloudflare-warp.yaml`, set the readiness flag, and switch every WARP fleet host.
Apply the preceding Mesh transport checks if the address itself remains unreachable.
Cloudflare hostname routes are unavailable under WireGuard, but direct IPv4 Mesh addresses and the
repository's local `.internal` aliases do not use that feature.
...
Diagnostic: `nc -vz <mesh-address> 22` succeeds and `ping -c 1 <mesh-address>` does not.
