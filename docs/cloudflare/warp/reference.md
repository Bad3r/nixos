...
| `connectOnBoot` | bool       | `true`                                               | Best-effort oneshot that requires an initial managed confirmation, then permits same-run connection retries only until a successful mismatch. It makes up to 30 connect/status attempts within a 120-second deadline and a 180-second start timeout. Runs once per boot with no retry timer. |
