...
| `connectOnBoot` | bool       | `true`                                               | Best-effort oneshot that verifies managed registration, then performs up to 30 connect/status attempts within a 120-second deadline and a 180-second start timeout. Runs once per boot with no retry timer. |
