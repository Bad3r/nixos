...
| AES-GCM tag failure on download     | A remote volume is truncated, mangled, or corrupted in transit.                          | Same as `MissingRemoteFiles`. Duplicati treats damaged volumes as missing because partial-block recovery is not supported.                   |
