...
**Ordering:** both policy files created here import
...
`_keepalive-key.nix` is generated by Step 1 below but committed by Task 10 Step 4, with the two files that first
import it, so that commit evaluates from a clean checkout.
