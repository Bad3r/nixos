...
`pre-commit run --all-files --hook-stage manual` at Task 13 Step 2, which is
where the hook runs.
...
Task 10 adds `_policy.nix` and `_keepalive-key.nix`.
...
- Create: `modules/browsers/webapps/_keepalive-key.nix` (generated by Task 11 Step 1, committed here with its first consumers)

- Create: `modules/browsers/webapps/_policy.nix`

- Create: `modules/browsers/webapps/_check-apps.nix`

**Ordering:** `modules/browsers/webapps/_keepalive-key.nix` is generated by
Task 11 Step 1. Run that step now, before Step 3 below, or this task cannot
evaluate: `_check-apps.nix` imports the file directly, and Task 9's `nixos.nix`
already does. `_policy.nix` does not import it, taking `keepAliveExtensionId`
as an argument instead, so Step 1 below is writable without it. The tasks stay
in this order because the policy generator is what explains why the key has to
be pinned at all.
...
_keepalive-key.nix ships here rather than with the extension builder because _check-apps.nix, added in this commit,
imports it, as does nixos.nix from the previous one, and a commit that evaluates only while the key sits untracked
in the working tree is not reproducible.
...
- Create: `modules/browsers/webapps/keepalive-id-check.nix`

`_keepalive-key.nix` is generated by Step 1 below but committed by Task 10 Step 4, alongside `_check-apps.nix`, so
that commit evaluates from a clean checkout. Its first importer is Task 9's `nixos.nix`, one commit earlier, which
is why that commit is the one phase 3 exception noted at Task 9 Step 3.
...
Validation: nix build of the extension for teams, asserting the manifest shape and the baked interval;
